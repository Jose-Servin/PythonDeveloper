...
We first define an asynchronous function counter that counts from 0 to a specified number (until), pausing for a short duration between each count. It measures the time slept during each iteration and prints it out.
...
We next have a standard `send_request()` function.
...
Lastly, in our `main()` function we have a call to an API and invoke our asynchronous counter. Here, we'll see how the `send_request()` function executes first and then the asynchronous counter is executed.
...
What we see here is our counter waiting for the API response but why should we wait to start the counter?

```terminal
python sync_to_async_before.py
Sending HTTP request
Got HTTP response with status 200
Started counter
0: Was asleep for 0.010275803000013184s
1: Was asleep for 0.011272838999957457s
2: Was asleep for 0.01025384300010046s
3: Was asleep for 0.011227138999856834s
4: Was asleep for 0.010687103000009301s
5: Was asleep for 0.011234295000122074s
6: Was asleep for 0.011271260999819788s
7: Was asleep for 0.010383467000110613s
8: Was asleep for 0.011247764000017924s
9: Was asleep for 0.011222631999999066s
```

To "fix" this, we will convert our `send_request()` function to an asynchronous function. The implementation for this is in file `Projects/NextLevelPython/sync_to_async_after.py`.

Here, we've added an asynchronous `send_async_request()` function that calls our synchronous `send_request()` function but uses the `async.to_thread()` call.

Note that this `to_thread()` follows the format `async.to_thread({function}, {arguments})`
...
Our `async main()` function can now use `asyncio.gather()` to call these 2 async functions concurrently.
...
From ChatGPT - Breaking down our new `async main()` function:

`asyncio.gather()` is used to concurrently execute multiple asynchronous tasks and gather their results.

In this specific case, `asyncio.gather()` is called with two tasks:

`send_async_request("https://www.arjancodes.com")`: This function sends an asynchronous HTTP request to a specified URL and returns the status code of the response.

`counter()`: This function is another asynchronous task that counts and prints the elapsed time. It doesn't return any value.

`status_code, _ = await asyncio.gather(...)`: This line awaits the completion of both asynchronous tasks (`send_async_request` and `counter`) and unpacks the results. However, the result of `counter()` is not needed in this context, so the `_` variable is used to "discard" it.

`status_code`: This variable will hold the result of `send_async_request`, which is the status code of the HTTP response.

`_`: This variable is used as a placeholder to discard the result of `counter()`. It's a common convention in Python to use _ as a variable name when you want to discard a value that you don't intend to use.

By using `status_code, _ = ...`, the code explicitly indicates that it's only interested in the status code returned by the HTTP request, and it discards the result of the `counter()` function. This helps improve code readability and clarifies the intention of the programmer.

Executing this new `sync_to_async_after.py` shows us how the counter and asynchronous send request occur concurrently.

```terminal
python sync_to_async_after.py
Sending HTTP request
Started counter
0: Was asleep for 0.01119170900028621s
1: Was asleep for 0.010734455999681813s
2: Was asleep for 0.010437648000333866s
3: Was asleep for 0.010650530000020808s
4: Was asleep for 0.010453288000007888s
5: Was asleep for 0.010201909999977943s
6: Was asleep for 0.011191608999979508s
7: Was asleep for 0.010364586999912717s
8: Was asleep for 0.011130609999781882s
9: Was asleep for 0.010734613000295212s
Got HTTP response with status 200.
```

## Iterators

At a high level, `iterators` in python are objects that represent streams of data. We are able to iterate over elements without having to know the underlying structure of our data. Iterators are also memory efficient because they don't store all elements in memory at once; instead, they are generated when requested.

Iterators are implemented using two methods: `__iter__()` and `__next__()`. The `__iter__()` method returns the iterator object itself and is called when the iterator is initialized. The `__next__()` method returns the next item in the sequence. When there are no more items to return, it raises the `StopIteration` exception.

After defining our Iterator, We can then use the built-in functions `iter()` and `next()`:

```python
my_list = [1, 2, 3, 4, 5]
my_iter = iter(my_list)  # Creating an iterator from a list
...
Implicitly, iterators are used in loops:

```python
my_list = [1, 2, 3, 4, 5]
for item in my_list:  # This implicitly creates an iterator and iterates over the elements
    print(item)
```

### Iter Method Sentinel Value

A sentinel value is a special value used to signal the end of a sequence or **stream of data**, especially in cases where the length of the sequence is not known in advance or where it is not feasible to check for the end condition in advance. We use the sentinel value to indicate the end of an iteration and prevent a `StopIteration` Exception.

For example, we'll consider the file `data.txt`
...
We can use a Sentinel value to stop reading lines when we encounter a specific marker.
...
with open("data.txt", "r") as file:
    ...
    for line in iter(read_lines(), sentinel):
        print(line)
```

#### Understanding `read_lines()`

The `read_lines()` generator function allows the caller to receive each line from a file one by one. i.e "The Generator function separates the concerns of reading and processing lines from the file from the iteration logic."

#### Generator Function Defined

Generator functions are a special type of function that generates a sequence of values using the yield statement, pausing execution between each yield statement.

This approach allows us to read lines from a text file until a specific marker is reached, without having to handle exceptions or read the entire file into memory at once.

We can also signal the end of the file by using the following sentinel value:
...
For this example, we'll be looking at the file `Projects/NextLevelPython/iterator_abstraction.py`.

We first start with defining a `LineItem` dataclass with a `total_price()` method:
...
We can see the abstraction applied thanks to Iterables in the `print_totals()` function.
...
Thanks to the `Iterable[LineItem]` type annotation, we are saying "items, is an Iterable of ANY type, not just a list". This will allow us to pass a tuple, dictionary, or any other object that is an `Iterable`.

So `line_items` can be a tuple or list
...
Why should we add this abstraction? Our `print_totals()` function does not care about the data structure we pass - rather, the only requirement is that there are `items` which means we can execute the `total_price()` method from our `LineItem` Dataclass.

### An introduction to Itertools

[Arjan Codes Iterator Video](https://www.youtube.com/watch?v=aumxFs2DO5o&ab_channel=ArjanCodes)

For this example, we'll be executing at the file `Projects/NextLevelPython/itertools_example.py`.

For now, the implementation for itertools should come from refactoring. Think about how we are iterating (for loops) and what calculations are being performed...Itertools might have a function that can help!
...
```text
Python generators are special functions that allow you to generate a sequence of values lazily, i.e., one at a time, rather than producing them all at once and storing them in memory. They enable efficient memory usage, particularly when dealing with large datasets or infinite sequences.
```

Generators are most commonly used when:

**Processing Large Datasets**: When working with large datasets that cannot fit entirely into memory, generators can process data one piece at a time, avoiding memory errors.

**Stream Processing**: Generators are useful for processing data streams or reading large files line by line without loading the entire file into memory.

**Infinite Sequences**: Generators are handy for generating infinite sequences of values, such as an endless stream of sensor data or an infinite sequence of numbers.

**Efficient Iteration**: If you need to iterate over a sequence of values only once and don't need to store the entire sequence in memory, generators provide a memory-efficient solution.
...
1. We use a `for` loop to iterate over each row in the CSV file. Instead of returning each row, we `yield` it. This makes the function a generator. Each time the loop iterates, it yields a row from the CSV file.

2. When the generator function is called, it returns a generator object, which can be iterated over.

3. Outside the function, we use a `for` loop to iterate over the generator object returned by `read_large_csv`. This iterates over the rows of the CSV file one by one.

4. In this example, we print each row, but you can perform any processing you need on the rows inside the loop.

### Generator Expressions

Similar to lambda function, we also the ability to create generators in a concise way without having to declare a separate function. 
...
# Define a generator expression to read the CSV file line by line
csv_generator = (row for row in csv.reader(open(file_path, 'r')))

# Example usage: Iterate over the generator to process each row
for row in csv_generator:
    print(row)

```

We first create a generator called `csv_generator` by using a generator expression. Next, we iterate through this generator in our `for` loop.

### Generators and Concurrency

See file `Projects/NextLevelPython/generator_asyncio.py`
